...
function initializeSocketHandlers(io) {
    io.on('connection', async (socket) => {
        ...
        try {
            const confirmedCount    = await db.pool.query('SELECT COUNT(*) FROM confirmados');
            const notConfirmedCount = await db.pool.query('SELECT COUNT(*) FROM no_confirmados');
            socket.emit('stats-update', {
                confirmed:    confirmedCount.rows[0].count,
                notConfirmed: notConfirmedCount.rows[0].count,
            });
        } catch (e) {
            console.error('Error al obtener contadores iniciales:', e.message);
        }
        ...
        socket.on('request-window-status', async ({ number }) => {
            try {
                const ws = await db.getConversationWindowState(number);
                let logType = 'log-info';
                if (ws.status === 'ACTIVE' || ws.status === 'COOL_DOWN') logType = 'log-success';
                if (ws.status === 'INACTIVE' || ws.status === 'EXPIRING_SOON') logType = 'log-warn';
                socket.emit('status-update', {
                    text: `[Consulta] Estado para ${number}: <strong>${ws.status}</strong> (${ws.details})`,
                    type: logType,
                });
            } catch (error) {
        ...
        socket.on('ver-confirmados', async () => {
            try {
                const result = await db.pool.query(
                    'SELECT cedula, fecha_nacimiento, numero_confirmado, confirmado_en FROM confirmados ORDER BY confirmado_en DESC'
                );
        ...
        socket.on('ver-no-confirmados', async () => {
            try {
                const result = await db.pool.query(
                    'SELECT cedula, fecha_nacimiento, numero_no_confirmado, registrado_en FROM no_confirmados ORDER BY registrado_en DESC'
                );
        ...
        socket.on('limpiar-confirmados', async () => {
            try {
                ...
                const notConfirmedCount = await db.pool.query('SELECT COUNT(*) FROM no_confirmados');
                io.emit('stats-update', { confirmed: 0, notConfirmed: notConfirmedCount.rows[0].count });
            } catch (dbError) {
        ...
        socket.on('limpiar-no-confirmados', async () => {
            try {
                ...
                const confirmedCount = await db.pool.query('SELECT COUNT(*) FROM confirmados');
                io.emit('stats-update', { confirmed: confirmedCount.rows[0].count, notConfirmed: 0 });
            } catch (dbError) {
