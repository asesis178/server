...
async function createDirectories() {
    const dirs = [
        UPLOADS_DIR, PENDING_DIR, ARCHIVED_DIR,
        CONFIRMED_ARCHIVE_DIR, NOT_CONFIRMED_ARCHIVE_DIR,
        ZIPS_DIR, TEMP_DIR, ASSETS_DIR,
    ];
...
async function checkActivationImage() {
    if (!fsSync.existsSync(activationImagePath)) {
        console.error(`🔥 ERROR FATAL: '${config.ACTIVATION_IMAGE_NAME}' no encontrado en /assets.`);
        process.exit(1);
    }
}
...
async function cleanupOldFiles(directory, maxAge) {
    try {
        ...
        if (err.code !== 'ENOENT') console.error(`Error al limpiar ${directory}:`, err);
    }
}
