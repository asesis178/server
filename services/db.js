...
async function initializeDatabase() {
    ...
    try {
        ...
        `);
        await client.query(`
            CREATE TABLE IF NOT EXISTS no_confirmados (
                id                    SERIAL PRIMARY KEY,
                numero_no_confirmado  VARCHAR(255) NOT NULL,
                cedula                VARCHAR(50) UNIQUE NOT NULL,
                fecha_nacimiento      DATE,
...
async function loadPendingTasks(io) {
    try {
        ...
        const res = await pool.query(
            "SELECT id, numero_destino, nombre_imagen FROM envios WHERE estado IN ('pending', 'procesando') ORDER BY id ASC"
        );
        if (res.rowCount > 0) {
...
async function getConversationWindowState(recipientNumber) {
    const result = await pool.query(
        'SELECT last_activation_time FROM conversation_windows WHERE recipient_number = $1',
        [recipientNumber]
    );
    if (result.rowCount === 0) return { status: 'INACTIVE', details: 'Nunca activada' };

    const lastActivation       = new Date(result.rows[0].last_activation_time).getTime();
    const minutesSinceActivation = (Date.now() - lastActivation) / 60000;
...
module.exports = { pool, initializeDatabase, loadPendingTasks, getConversationWindowState };
